...
const CONFIG = {
  ...
  // each entry needs a photo (src) and EITHER a text wish OR a wishGif (not both).
  // to show a gif instead of text, just replace "wish" with "wishGif" and point it at your .gif file:
  //   { src:"images/tree3.jpg", wishGif:"images/tree3-wish.gif" }
  treePhotos: [
    {
    ...
    {
      src: "./tree photos/3.jpg",
      wish: "Trust me when I say YOu are too GOOD at making those weird face's （笑）",
    },
    { src: "./tree photos/4.jpg", wishGif: "./hot.gif" },
    {
      src: "./tree photos/14.jpg",
      wish: "I know you won't trust me when I say you look STUNNING even without makeup",
    },
    ...
    {
      src: "./tree photos/12.jpg",
      wish: "Remember this one?? I will still say your best version was this one wait or is it just me ,could it be that I have a thing for blondie's!!??",
    },
    { src: "./tree photos/13.jpg", wish: "The only photo I have with you." },
    { src: "./tree photos/11.jpg", wish: "ANGEL!!!" },
  ],
};
...
CONFIG.treePhotos.forEach((item, i) => {
  ...
  btn.style.top = pos[1] + "%";
  // no image shown here on purpose — the photo only appears once clicked
...
// mobile: swipe right or swipe down to return to the tree
...
zoomOverlay.addEventListener(
  ...
  (e) => {
    ...
    const swipeRight = dx > 80 && Math.abs(dy) < 60;
    const swipeDown = dy > 80 && Math.abs(dx) < 60;
    if (swipeRight || swipeDown) closeZoom();
  },
  { passive: true },
);
...
function openZoom(item, sourceEl) {
  ...
  // if this photo has a wishGif, show the gif; otherwise show the text wish
  if (item.wishGif) {
    zoomWish.innerHTML = `<img src="${item.wishGif}" alt="wish">`;
  } else {
    zoomWish.innerHTML = `<p>${item.wish || ""}</p>`;
  }
